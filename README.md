# ParragaMatias26
# 👋 ¡Hola! Soy Matías Párraga  
🎮 Game Designer & Programador de Videojuegos

---

## Sobre mí  
Apasionado por el desarrollo de videojuegos que cuentan historias, exploran mundos y proponen mecánicas innovadoras.  
Me especializo en diseño de sistemas de juego, programación en C# (Unity), y en llevar una idea desde el prototipo hasta la experiencia.

---

## Mis valores como diseñador  
- Diseñar mecánicas que sean **intuitivas y significativas** para el jugador.  
- Priorizar la **coherencia narrativa y mecánica** en cada proyecto.  
- Trabajar con código limpio, modular y escalable.  
- Fomentar el **aprendizaje iterativo**: prototipar, testear, refinar.

---

## Proyectos destacados  
| Proyecto | Rol / Tecnologías | Descripción breve |
|----------|-------------------|-------------------|
| **[Beyond The Illusions]** | Programación · C# · Unity | Una aventura tanto tierna como grotesca donde nuestra percepcion de la realidad ira variando a lo largo de la aventura para lograr superar obstaculos. |
| **[Fluffy Search]** | Programación · C# · Unity | Un pequeño proyecto desarrollado durante mis estudios de desarrollo de videojuegos, centrado en la implementación de nuevos patrones de diseño y estructuras de código para mejorar la arquitectura del juego. |
| **[Mapacheff]** | Programacíon · C# · Unity | Proyecto desarrollado para el curso de Diseño de Niveles, centrado en crear una experiencia visualmente atractiva con una curva de aprendizaje fluida a lo largo de múltiples niveles. |

Podés ver cada repositorio haciendo clic en los enlaces arriba.

---

## Tecnologías y herramientas  
![Unity](https://img.shields.io/badge/Engine-Unity-000?logo=unity)  
![C#](https://img.shields.io/badge/Language-C%23-239120?logo=c-sharp)   
![Visual Studio](https://img.shields.io/badge/IDE-Visual%20Studio-5C2D91?logo=visual-studio)  

---

## ¿Qué busco?  
Estoy abierto a colaborar en equipos que desarrollen videojuegos independientes, prototipos experimentales o proyectos narrativos. Me interesa asumir roles donde combine **diseño de mecánicas** y **programación** para generar experiencias memorables.

---

## 📫 Contacto  
- LinkedIn: [linkedin.com/in/MatiasParraga](https://www.linkedin.com/in/matías-párraga-0b7077323)  
- Email: matias.parraga.trabajo@gmail.com
- Itch.io: [MyM_IndieDevelopment.itch.io]([https://mym-indiedevelopment.itch.io)  
